...
import { Building2, Users, Medal, Plus, Clock, Eye, Star, Pencil, ToggleLeft, ToggleRight, Loader2 } from "@lucide/vue"
...
async function toggleStatus(need) {
  const newStatus = need.status === "open" ? "closed" : "open"
  toggling.value[need.id] = true
  try {
    await api.patch(`/internships/needs/${need.id}/status`, { status: newStatus })
    const idx = allNeeds.value.findIndex(n => n.id === need.id)
    if (idx !== -1) allNeeds.value[idx].status = newStatus
  } catch (e) {
    console.error(e)
  }
  toggling.value[need.id] = false
}
  ...
  <div class="space-y-6 max-w-4xl">
    ...
    <Card class="p-6">
      ...
      <div v-if="allNeeds.filter(n => n.umkmId === umkmProfileId).length > 0" class="space-y-3">
        ...
        >
          <div class="min-w-0 flex-1 cursor-pointer hover:opacity-75" @click="router.push(`/internship-needs/${n.id}/edit`)">
          ...
          <div class="flex items-center gap-1.5 flex-shrink-0 ml-2">
            <Button size="sm" variant="ghost" class="h-7 w-7 p-0" @click="router.push(`/internship-needs/${n.id}/edit`)">
              <Pencil class="h-3.5 w-3.5" />
            </Button>
            <Button v-if="n.status !== 'filled'" size="sm" variant="ghost" class="h-7 w-7 p-0" :disabled="toggling[n.id]" @click="toggleStatus(n)">
              <Loader2 v-if="toggling[n.id]" class="h-3.5 w-3.5 animate-spin" />
              <ToggleLeft v-else-if="n.status === 'open'" class="h-3.5 w-3.5" />
              <ToggleRight v-else class="h-3.5 w-3.5" />
            </Button>
            <Badge :variant="statusColor[n.status] || 'outline'" class="text-[10px]">{{ n.status }}</Badge>
          </div>
        </div>
      </div>
    ...
    <Card class="p-6">
      <h2 class="font-semibold mb-4">Magang Selesai</h2>
      <div v-if="completedInternships.length > 0" class="space-y-2">
        <div
          v-for="m in completedInternships"
          :key="m.id"
          class="flex items-center justify-between border border-border rounded-lg p-3 cursor-pointer hover:bg-secondary/20 transition-colors"
          @click="router.push(`/internships/${m.id}`)"
        >
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            <Button size="sm" variant="ghost" @click.stop="router.push(`/internships/${m.id}/evaluate`)">
              <Star class="h-4 w-4 mr-1" />Beri Ulasan
            </Button>
            <Badge variant="secondary" class="text-[10px]">completed</Badge>
          </div>
        </div>
      </div>
      <div v-else class="text-center py-6 text-muted-foreground">
        <Medal class="h-8 w-8 mx-auto mb-2 opacity-50" />
        <p class="text-sm">Belum ada magang selesai</p>
      </div>
    </Card>
  </div>
</template>
